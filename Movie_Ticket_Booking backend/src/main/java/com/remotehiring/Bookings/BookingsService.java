...
//		mailservice.sendEmail(currentBooking);
		...
		int vehicleCost =  vehicleservice.getVehicleCost(currBooking.getVehicle_type());

		//String locationname = locationservice.getLocation_name(currBooking.getLocationid());
		
		int cost = duration*vehicleCost;
		
		currBooking.setCost(String.valueOf(cost));

		//currBooking.setLocation_name(locationname);
